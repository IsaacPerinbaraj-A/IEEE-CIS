...
Three projects in one repository, joined by npm workspaces: `frontend/` (the website and `/admin`, Vite → Vercel),
`backend/` (the admin server, Express → Render) and `shared/` (content types and rules, used by both). One
`npm install` at the top installs all three, and every command below is run from the top folder.
...
- Content is edited by rotating student office bearers, so content must stay in `frontend/src/data/*.json`, never hardcoded in pages.
  After go-live the database is the source of truth: every Vercel build overwrites `frontend/src/data/*.json` (and adds photos to
  `frontend/public/images`) with the published content, so the repo copy is only the starting copy and for local development.
- Home hero, About the Society and What We Do copy live in `frontend/src/data/home.json` (`**bold**` supported via `richText`).
...
- The admin lives at `/admin` (`frontend/src/admin/`, lazy-loaded) and is database-backed. The browser calls `/api/*` on the
  site's own origin; Vercel rewrites it to the admin server on Render (`backend/`, Express 5 + the `mongodb` driver),
  which stores accounts, sessions, every release, photos and the activity log in MongoDB Atlas (M0). Publish saves a
  numbered release, then calls Vercel's Deploy Hook; the build pulls the release (`frontend/scripts/fetch-content.ts`). GitHub
...
- `npm run server` starts the admin server (reads `.env.local`; without `MONGODB_URI` it runs but database routes
  answer 503). `npm run test:server` runs the node:test suites in `shared/`, `backend/` and `frontend/scripts/` (no database).
- `frontend/scripts/fetch-content.ts` does nothing without `CONTENT_EXPORT_URL`, so local builds use `frontend/src/data` as it is. With
  it set it overwrites `frontend/src/data/*.json`; don't commit that output.
- Test the home intro with a hard refresh on `/`. It does not replay when navigating to Home inside the site.

## Conventions

- Colours and fonts come from `frontend/tailwind.config.js` tokens (`ink`, `panel`, `raised`, `line`, `cream`, `mute`,
  `violet`, `gold`) and classes in `frontend/src/index.css` (`.wrap`, `.btn-gold`, `.btn-ghost`, `.chip`, `.h-page`,
...
- Home layout: side-by-side vs stacked is decided by one media query that must stay identical in
  `SIDE_QUERY` (`frontend/src/components/particles/layout.ts`) and the `@media` block in `frontend/src/index.css`.
...
- Code layout: `shared/` is used by the admin, the server and the build, so it must not use Node-only or browser-only
  APIs; `frontend/` never imports `backend/`, and `backend/` never imports `frontend/` (ESLint enforces all three). Both
  import `shared/` by relative path (`../../shared/x.ts`), never by package name. Node runs
  `backend/` and `frontend/scripts/` TypeScript directly: erasable syntax only, relative imports end in `.ts`, `import type` for types.
- `/admin` is served from `frontend/dist/admin.html` (index.html without inline scripts, made by the `adminHtml` plugin in
  `frontend/vite.config.ts`) under a strict CSP in `vercel.json` (`script-src 'self'`, `connect-src 'self'`). Never add inline
...
- `frontend/tsconfig.json` has `erasableSyntaxOnly`: no constructor parameter properties; declare fields and assign them.
- ESLint `react-refresh/only-export-components`: component files may only export components. Shared constants
  and icon maps live in `frontend/src/lib/` (e.g. `frontend/src/lib/icons.ts`).
- Keep `lucide-react` at 0.294.x. Newer versions removed the LinkedIn/GitHub/Instagram brand icons used here.
- The intro depends on three places staying in sync: `frontend/public/intro.js` (loaded by a blocking `<script src>` at the
  start of `<body>` in `index.html`; sets `body[data-intro="on"]` before first paint on `/`; a file, not inline, so the
  admin's CSP allows it), `frontend/src/components/particles/ParticleStory.tsx`, and the `body[data-intro]` rules in
  `frontend/src/index.css` (states `on` / `leaving` / `done`). Any fallback path must set
...
- Particle engine (`frontend/src/components/particles/engine.ts`) is raw WebGL1, no three.js. Shapes are `ShapeSpec`
...
- Images: team photos 480x480 WebP with the face centred; posters max 900 px wide WebP; lowercase-dash filenames.
  ...
  no `?v=`) and copied into `frontend/public/images` at build time. Logos (`frontend/public/brand`) stay in the code.
