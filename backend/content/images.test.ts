...
test("every photo in the starting copy passes the upload checks", async () => {
  ...
  for (const folder of IMAGE_FOLDERS) {
    let names: string[] = [];
    try { names = await readdir(join(contentRoot(), "public", "images", folder)); } catch { continue; }
    for (const name of names) {
      const type = typeFromFileName(name);
      if (!type) continue;
      const r = checkImage(await readFile(join(contentRoot(), "public", "images", folder, name)), type);
