...
/** Accessible modal: focuses itself, closes on Esc, keeps the page behind from scrolling. */
export function Modal({ title, onClose, children, footer, wide = false }: { title: string; onClose: () => void; children: ReactNode; footer?: ReactNode; wide?: boolean }) {
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => { const all = document.querySelectorAll("[role=dialog]"); if (e.key === "Escape" && all[all.length - 1] === ref.current) onClose(); };
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-end justify-center bg-black/60 backdrop-blur-sm sm:items-center sm:p-6" onMouseDown={e => { if (e.target === e.currentTarget) onClose(); }}>
