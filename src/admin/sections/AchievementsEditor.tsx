...
export default function AchievementsEditor() {
  ...
  const [busy, setBusy] = useState(-1), [imgError, setImgError] = useState<{ i: number; text: string } | null>(null);
        ...
        {list.map((a, i) => (
          <section key={i} className="adm-card grid gap-4 sm:grid-cols-2">
            ...
            <ImageInput label="Photo (optional)" src={preview(a.image)} busy={busy === i} onRemove={() => edit(x => { x[i].image = ""; })}
              hint={imgError?.i === i ? imgError.text : undefined}
              onFile={async f => {
                setBusy(i); setImgError(null);
                try { const src = await addImage("achievements", slugify(a.title) || `achievement-${i + 1}`, await renderFit(await fileToImage(f), 1200, 900)); edit(x => { x[i].image = src; }); }
                catch (e) { setImgError({ i, text: (e as Error).message }); }
                finally { setBusy(-1); }
              }} />
